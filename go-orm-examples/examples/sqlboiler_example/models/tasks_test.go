...
func testTaskToManyParentTaskTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.ParentTaskID, a.ID)
	queries.Assign(&c.ParentTaskID, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.ParentTaskTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.ParentTaskID, b.ParentTaskID) {
			bFound = true
		}
		if queries.Equal(v.ParentTaskID, c.ParentTaskID) {
			cFound = true
		}
	}
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadParentTaskTasks(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ParentTaskTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ParentTaskTasks = nil
	if err = a.L.LoadParentTaskTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ParentTaskTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTaskToManyAddOpParentTaskTasks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddParentTaskTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.ParentTaskID) {
			t.Error("foreign key was wrong value", a.ID, first.ParentTaskID)
		}
		if !queries.Equal(a.ID, second.ParentTaskID) {
			t.Error("foreign key was wrong value", a.ID, second.ParentTaskID)
		}

		if first.R.ParentTask != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.ParentTask != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.ParentTaskTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ParentTaskTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ParentTaskTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testTaskToManySetOpParentTaskTasks(t *testing.T) {
	var err error
	...
	err = a.SetParentTaskTasks(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetParentTaskTasks(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.ParentTaskID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.ParentTaskID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.ParentTaskID) {
		t.Error("foreign key was wrong value", a.ID, d.ParentTaskID)
	}
	if !queries.Equal(a.ID, e.ParentTaskID) {
		t.Error("foreign key was wrong value", a.ID, e.ParentTaskID)
	}

	if b.R.ParentTask != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.ParentTask != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.ParentTask != &a {
	...
	if a.R.ParentTaskTasks[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.ParentTaskTasks[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testTaskToManyRemoveOpParentTaskTasks(t *testing.T) {
	var err error
	...
	err = a.AddParentTaskTasks(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveParentTaskTasks(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.ParentTaskTasks) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.ParentTaskTasks[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.ParentTaskTasks[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testTaskToOneTaskUsingParentTask(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.ParentTaskID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ParentTask().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadParentTask(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ParentTask == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ParentTask = nil
	if err = local.L.LoadParentTask(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ParentTask == nil {
		t.Error("struct should have been eager loaded")
...
func testTaskToOneSetOpTaskUsingParentTask(t *testing.T) {
	var err error
	...
	for i, x := range []*Task{&b, &c} {
		err = a.SetParentTask(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.ParentTaskTasks[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.ParentTaskID, x.ID) {
			t.Error("foreign key was wrong value", a.ParentTaskID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ParentTaskID))
		reflect.Indirect(reflect.ValueOf(&a.ParentTaskID)).Set(zero)
		...
		if !queries.Equal(a.ParentTaskID, x.ID) {
			t.Error("foreign key was wrong value", a.ParentTaskID, x.ID)
		}
	}
}

func testTaskToOneRemoveOpTaskUsingParentTask(t *testing.T) {
	var err error
	...
	if err = a.SetParentTask(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveParentTask(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.ParentTask().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.ParentTask != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.ParentTaskID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.ParentTaskTasks) != 0 {
...
var (
	taskDBTypes = map[string]string{`ID`: `bigint`, `Title`: `character varying`, `Note`: `text`, `Status`: `enum.task_status('todo','in_progress','done')`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `UserID`: `bigint`, `ParentTaskID`: `bigint`}
	_           = bytes.MinRead
)
