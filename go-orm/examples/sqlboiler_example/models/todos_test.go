...
var (
	todoDBTypes = map[string]string{`ID`: `bigint`, `Title`: `character varying`, `Description`: `text`, `Status`: `enum.status('todo','in_progress','done')`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `UserID`: `bigint`}
	_           = bytes.MinRead
)
